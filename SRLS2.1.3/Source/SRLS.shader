...
Shader "Unlit/SRLS1.0"
{
    Properties
    {
        ...
        _SkinSaturation("SkinSaturation",Range(0.0,5.0))=1.1//Seems the skin is too pale in the rendering outcome,so this value aims to make the skin more flush
        
        [Header(Metal and Speculer)]
        _MetalMap ("MetalMap",2D) = "white"{}
        _Smoothness("Smoothness",Float) = 4
        _NonMetalSpecular("NonMetalSpecular",Range(0.0,10.0))=2
        _MetalSpecular("MetalSpecular",Range(0.0,10.0))=2
        ...
        [Header(OutLine)]
        _OutlineWidth("OutlineWidth",Float) = 0.35  
        ...
        _outlineColor5( "描边颜色6" , color) = (0.5, 1.0, 1.0, 0.0)
        _outlineColor6( "描边颜色7" , color) = (0.5, 0.5, 1.0, 0.0)
        _outlineColor7( "描边颜色8" , color) = (1.0, 0.5, 1.0, 0.0)
        ...
        _OffsetMul("RimWidth:offsetMul", Range(0, 100)) = 0.002
        _rimThreshold("RimThreshold", Range(0, 1)) = 0.542
        _RimColor("RimColor",Color)=(1,1,1,1)
        
        [Header(Stencil )]
        _AlphaForStencil("Eyebrows_Through_The_Hair_Effect",Range(0,1))=0.5
        ...
        //[HideInInspector] _Front("_Front",Color) = (0,0,1,0)
        //[HideInInspector] _Right("_Right",Color) = (-1,0,0,0)
    }
    SubShader
    {
        ...
        Pass //Back
        {
            ...
            v2f vert(a2v input)
            {
                ...
                output.positionVS.rgb = vertex_position_inputs.positionVS;
        ...
        Pass//DrawOverLay
        {
            Tags{"LightMode" = "drawoverlay"}
            Cull Back
            Stencil
            {
                ref [StencilRefOverlay]
                Comp [StencilCompOverlay]
            }
            ...
            HLSLPROGRAM
                ...
                v2f vert(a2v input)
                {
                    ...
                    output.TtoW0 = float4(TangentDirWS.x,BinormalDirWS.x,NormalDirWS.x,positionWS.x);
                    output.TtoW1 = float4(TangentDirWS.y,BinormalDirWS.y,NormalDirWS.y,positionWS.z);
                    output.TtoW2 = float4(TangentDirWS.z,BinormalDirWS.z,NormalDirWS.z,positionWS.y);
                    output.positionVS.rgb = vertex_position_inputs.positionVS;
                    output.uv=TRANSFORM_TEX(input.uv,_BaseMap);
                    output.positionNDC = vertex_position_inputs.positionNDC;
